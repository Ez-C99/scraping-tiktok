...
PROXY_LIST = ["http://188.68.52.244:80"]
...
async def test_proxy(session, proxy):
    ...
    try:
        ...
        async with session.get(
            "http://httpbin.org/ip", proxy=proxy, timeout=5
        ) as response:
            ...
            videos.append(
                {
                    ...
                    "like_count": video_stats.get("diggCount"),
...
async def main_async():
    ...
    if user_data:
        ...
        user_df.to_csv(f"reports/{username}_info.csv", index=False)

        # Save videos data
        if videos_data and videos_data:
            videos_df = pd.DataFrame(videos_data)
            logging.info(f"\nFound {len(videos_data)} videos:")
            logging.info(videos_df.head())
            videos_df.to_csv(f"reports/{username}_videos.csv", index=False)
        else:
